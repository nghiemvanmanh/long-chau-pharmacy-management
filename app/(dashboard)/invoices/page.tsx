...
import { DataTable } from "@/components/ui/data-table" // Ensure this is a named import if DataTable is not default
...
export default function InvoicesPage() {
  ...
  const filteredInvoices = useMemo(() => {
    return invoices.filter((invoice) => {
      ...
      const matchesCreatedBy = createdByFilter === "all" || invoice.createdBy === createdByFilter

      const matchesDateRange =
        (!dateRange.from || new Date(invoice.createdAt) >= new Date(dateRange.from)) &&
        (!dateRange.to || new Date(invoice.createdAt) <= new Date(dateRange.to + "T23:59:59"))
  ...
  }, [invoices, searchTerm, saleTypeFilter, statusFilter, paymentStatusFilter, createdByFilter, dateRange])
  ...
  const handleFormSubmit = async (invoiceData: Omit<Invoice, "id" | "invoiceNumber" | "createdAt" | "updatedAt">) => {
    if (selectedInvoice) {
  ...
  const invoiceColumns = [
    {
      ...
      render: (value: string, row: Invoice) => (
        <div className="flex items-center gap-2">
          ...
          {new Date().getTime() - new Date(row.createdAt).getTime() < 30 * 60 * 1000 && (
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" title="Hóa đơn mới" />
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (value: string) => (
        <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">
          {value === "cash" ? "Tiền mặt" : value === "card" ? "Thẻ" : "Chuyển khoản"}
        </Badge>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (value: string) => (
        <div>
          <span className="text-sm">{new Date(value).toLocaleString("vi-VN")}</span>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.3 }}>
        <DataTable
          ...
          customActions={(invoice: Invoice) => (
            <Button variant="ghost" size="icon" onClick={() => handleView(invoice)} title="Xem chi tiết">
